...
AccessiText is a comprehensive text accessibility analyzer designed to help content creators make their writing accessible to people with cognitive disabilities, low literacy levels, and non-native English speakers. It addresses a critical gap in digital inclusion: most web content is written at a college reading level, effectively excluding millions of people who need simpler language.

The tool performs multi-dimensional readability analysis using four established algorithms (Flesch-Kincaid Grade Level, Flesch Reading Ease, Coleman-Liau Index, and Automated Readability Index). Beyond scoring, it actively identifies specific accessibility barriers: complex vocabulary with simpler alternatives (100+ word mappings), passive voice constructions, overly long sentences, and jargon.

AccessiText generates an overall accessibility score (0-100) with letter grades, maps findings to WCAG 2.1 cognitive accessibility guidelines (Success Criterion 3.1.5), and produces actionable recommendations prioritized by impact. It also generates a simplified version of the input text by automatically substituting complex words and suggesting sentence breaks.

The architecture uses four modular classes: TextAnalyzer (NLP and metrics), AccessibilityScorer (scoring and grading), SimplificationEngine (text transformation), and the main AccessiText orchestrator. This separation enables easy extension — new readability formulas, language support, or domain-specific vocabulary can be added independently.

Real-world impact: government agencies, healthcare providers, and educators can use AccessiText to ensure their communications reach everyone, not just highly literate audiences. The tool aligns with the W3C Cognitive Accessibility Guidelines and supports the broader mission of digital inclusion.
